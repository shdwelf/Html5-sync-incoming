...
(function (global) {
  ...
  // Gas Fee (TAX) tile: 10% of cash, with a 40 floor. The engine clamps the
  // charge to the player's balance so cash can never go negative.
  const TAX_RATE = 0.1;
  const TAX_FLOOR = 40;

  // Exchange (DEX/CEX) rent by number of EXCH tiles owned. There are exactly
  // three EXCH tiles on the board, so the table has exactly three tiers.
  const EXCH_RENT = [25, 50, 100];
  ...
  // Defensive: JS `%` keeps the sign of the dividend, so a negative index would
  // fall off the front of TILES and return undefined.
  function tileByIndex(i) { return TILES[((i % BOARD_SIZE) + BOARD_SIZE) % BOARD_SIZE]; }

  global.CryptoBoard = {
    TILES, POSITIONS, EVENT_DECK, TOKENS, PLAYER_COLORS,
    GO_SALARY, POT_START, JAIL_INDEX, START_CASH, BOARD_SIZE,
    TAX_RATE, TAX_FLOOR, EXCH_RENT,
    type: T, tileByIndex,
  };
