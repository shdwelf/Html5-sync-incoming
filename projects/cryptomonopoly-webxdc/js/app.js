...
(function (global) {
  ...
  function renderLobby() {
    ...
    if (webxdcMode) {
      ...
      tokensHost.innerHTML = B.TOKENS.map((t) =>
        `<button class="token" data-tok="${escAttr(t.glyph)}" title="${escAttr(t.label)}">${esc(t.glyph)}</button>`).join("");
  ...
  function renderLocalSeats() {
    ...
    host.innerHTML = lobbySeats.map((s, i) => `
      <div class="seat">
        <span class="seat-token">${esc(s.token)}</span>
  ...
  function joinChip(p, me) {
    return `<div class="join" style="--pc:${escColor(p.color)}">
      <span class="join-tok">${esc(p.token)}</span><span>${escAttr(p.name)}</span>${me ? " <em>(you)</em>" : ""}
    </div>`;
  }
  ...
  function playersStrip() {
    return state.order.map((id, i) => {
      ...
      return `<div class="pbadge ${state.turn === i ? "active" : ""} ${p.alive ? "" : "dead"}" style="--pc:${escColor(p.color)}">
        <span class="pb-tok">${esc(p.token)}</span>
  ...
  function tileHtml(idx, r, c) {
    ...
    }).map((id) => `<span class="marker" style="--mc:${escColor(state.players[id].color)}">${esc(state.players[id].token)}</span>`).join("");
    ...
    if (ownable) {
      ...
      if (owner) body += `<div class="tt-owner" style="background:${escColor(owner.color)}">${esc(owner.token)}</div>`;
    } else {
      body = `<div class="tt-sym big">${t.sym}</div><div class="tt-name">${t.name}</div>`;
    }
    const colorTop = ownable && t.color ? ` style="--ct:${escColor(t.color)}"` : "";
  ...
  function centerHtml(cur, operate) {
    ...
    return `<div class="side">
      <div class="panel who">
        <span class="who-tok" style="--mc:${escColor(cur.color)}">${esc(cur.token)}</span>
        <div>
  ...
  function resetAll() {
    seq = 0;
    // Route RESET through the transport so every peer (webxdc) or every mirror
    // tab (BroadcastChannel) returns to the lobby together. Resetting only the
    // local copy used to leave the rest of the table on the finished game.
    if (transport) transport.broadcast({ type: "RESET" });
    else { state = Engine.initialState(); render(); }
  }
  ...
  // Colours land inside style="..." attributes. They normally come from
  // B.PLAYER_COLORS, but in webxdc mode a remote peer supplies them, so only a
  // conservative colour token is allowed through (the engine also validates on
  // the way into state — this is defence in depth at the render boundary).
  const COLOR_RE = /^(#[0-9a-fA-F]{3,8}|[a-zA-Z]{3,20}|rgba?\(\s*[\d.]+\s*,\s*[\d.]+\s*,\s*[\d.]+\s*(,\s*[\d.]+\s*)?\))$/;
  function escColor(c) { const s = String(c == null ? "" : c).trim(); return COLOR_RE.test(s) ? s : "#888888"; }
